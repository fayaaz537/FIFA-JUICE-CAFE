...
function setup() {
  ...
  if (brand) brand.textContent = restaurant.name;
  if (hero) hero.textContent = restaurant.name;
  if (footerTitle) footerTitle.textContent = restaurant.name;

  const footerAddress =
    document.querySelector("footer p:nth-of-type(1)");

  const footerPhone =
    document.querySelector("footer p:nth-of-type(2)");
...
function cats() {
  ...
  const categories = [
    "All",
    ...new Set(menu.map(x => x.cat))
  ];

  document.getElementById("categories").innerHTML =
    categories.map(c => `
      <button
        class="cat ${c === active ? "active" : ""}"
        onclick="active='${c}';cats();render()"
      >
        ${c}
      </button>
    `).join("");
...
function update() {
  ...
  const output = Object.entries(cart)
    .map(([id, quantity]) => {
...
async function startOrderTracking(orderId, orderNumber) {
  ...
  try {
    ...
    const orderNoElement =
      document.getElementById("trackOrderNo");

    if (orderNoElement) {
      orderNoElement.textContent = orderNumber || "";
    }
...
document
  ...
  .addEventListener("submit", async event => {
    ...
    try {
      ...
      /* Save order for customer tracking */
      ...
      alert(
        "Order submit cheyyan pattiyilla. Please try again."
      );
